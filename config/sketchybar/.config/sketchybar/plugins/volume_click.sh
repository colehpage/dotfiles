  ...
  while IFS= read -r device; do
    ...
    args+=(--add item volume.device.$COUNTER popup."$NAME" \
           --set volume.device.$COUNTER label="${device}" \
                                        label.color="$COLOR" \
                 click_script="SwitchAudioSource -s \"${device}\" && sketchybar --set /volume.device\.*/ label.color=$COLOR_GREY --set \$NAME label.color=$COLOR_WHITE --set $NAME popup.drawing=off")
    COUNTER=$((COUNTER+1))
